...
class Binary {
  ...
  eval() {
    const expr = `${this.left.eval()} ${this.op.value} ${this.right.eval()}`;
    console.log('evaluating: ', expr, '=', eval(expr));
    return eval(expr);
  }
  toString() { return `(${this.left} ${this.op.value} ${this.right})`; }
}
...
function parseEnclosed(tokens) {
  tokens.consume(PUNCTUATION, OPEN_EXPR);
  ...
  tokens.consume(PUNCTUATION, CLOSE_EXPR);
  return ast;
}
...
function isNextHigherPrecedenceBinary(priorOp, nextToken) {
  return nextToken.type === BINARY_OP
    // more operators would require more complex precedence below
    && priorOp.value === '*' && nextToken.value === '+';
}

function parseBinary(tokens, left) {
  const currentOp = tokens.consume(BINARY_OP);
  let right = parseValue(tokens);

  while (!tokens.eof() && isNextHigherPrecedenceBinary(currentOp, tokens.peek())) {
    right = parseBinary(tokens, right)
  }
  return new Binary(left, currentOp, right);
}
...
function parseExpression(tokens) {
  ...
  while (!tokens.eof()) {
    ...
    if (token.type === BINARY_OP) {
      value = parseBinary(tokens, value); // OP is still current
    } else {
      return value;
    }
  }
